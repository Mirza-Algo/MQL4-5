...
#property version   "6.00" //Document version 23.10.21
#property strict
#include <stdlib.mqh>
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
   ...
   {
      BuyTicket = OpenBuyOrder(Symbol(), lots[0], 0, MagicNumber, "Alpha Winner V6.00");
      if(BuyTicket > 0)
      {
      ...
      SellTicket = OpenSellOrder(Symbol(), lots[0], 0, MagicNumber, "Alpha Winner V6.00");
      if(SellTicket > 0)
      {
...
//+------------------------------------------------------------------+
//| Initial Order Take Profit Check                                  |
//+------------------------------------------------------------------+

if(BuyMarketCount(Symbol(), MagicNumber)==1)
{
   ...
   if(OrderProfit()>0 && MathAbs(NormalizeDouble(OrderOpenPrice()-Bid, _Digits)) > 1.0)
   {
         ...
         if(Closed == false)
         {
            ...
            Alert(ErrAlert);
            string ErrLog = StringConcatenate("Bid: ",MarketInfo(Symbol(),MODE_BID), " Ask: ",MarketInfo(Symbol(),MODE_ASK)," Ticket: ",CloseTicket);
            Print(ErrLog);
         }
   };
};
if(SellMarketCount(Symbol(), MagicNumber)==1)
{
   for(int i=0; i<=OrdersTotal()-1; i++)
   {
      bool selected = OrderSelect(i, SELECT_BY_POS);
   ...
   if(OrderProfit()>0 && MathAbs(NormalizeDouble(OrderOpenPrice()-Ask, _Digits)) > 1.0)
   {
         int CloseTicket = OrderTicket();
         while(IsTradeContextBusy()) Sleep(10);
         double ClosePrice;
...
//+------------------------------------------------------------------+
   if(OrdersTotal()>=2)
   {
      for(int i=OrdersTotal()-1; i>=0; i--)
      {
        ...
        {
           BuyTicket = OpenBuyOrder(Symbol(), lots[BuyMarketCount(Symbol(), MagicNumber)], 0, MagicNumber, "Alpha Winner V6.00");
           if(BuyTicket>0)
           {
              bool selected = OrderSelect(BuyTicket, SELECT_BY_TICKET);
              double OpenPrice = OrderOpenPrice();
      ...
      for(int i=OrdersTotal()-1; i>=0; i--)
      {
        ...
        if(OrderProfit()<0 && MathAbs(NormalizeDouble(OrderOpenPrice()-Ask, _Digits)) >= 7.0)
        {
           SellTicket = OpenSellOrder(Symbol(), lots[SellMarketCount(Symbol(), MagicNumber)], 0, MagicNumber, "Alpha Winner V6.00");
           if(SellTicket>0)
           { 
             bool selected = OrderSelect(SellTicket, SELECT_BY_TICKET);
             double OpenPrice = OrderOpenPrice();
...
//+------------------------------------------------------------------+
   ...
   if(TotalOrderCount(Symbol(), MagicNumber)>2)
   {
      ...
      for(int i=OrdersTotal()-1; i>=0; i--)
      {
         ...
         {
            double PriceDrop = OrderOpenPrice() - CurrentPrice;
            double PartialCalcAlphaUnitsTP = OrderLots()*PriceDrop;
            CalcAlphaUnitsTP += PartialCalcAlphaUnitsTP;
         };
      };

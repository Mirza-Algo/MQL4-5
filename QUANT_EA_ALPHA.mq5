...
//----Calculate and Verify Lot Size------------------
   ...
   maxlot  = AccountInfoDouble(ACCOUNT_MARGIN_FREE) / MarginRequired(_Symbol);
   maxlot = StringToDouble(DoubleToString(MathFloor(maxlot/lotstep)*lotstep,2));
   maxlot = maxlot - 0.1;
...
double MarginRequired(string symbol)
{
   ...
   if(!OrderCalcMargin(type, symbol, 1.0, price, rate))  return(0.0);
   return(rate);
}
...
#property version   "5.00" //Document version 23.10.21
#property strict
#include <stdlib.mqh>
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
   ...
   {
      BuyTicket = OpenBuyOrder(Symbol(), lots[0], 0, MagicNumber, "Alpha Winner V5.00");
      if(BuyTicket > 0)
      {
      ...
      SellTicket = OpenSellOrder(Symbol(), lots[0], 0, MagicNumber, "Alpha Winner V5.00");
      if(SellTicket > 0)
      {
...
//+------------------------------------------------------------------+
   if(OrdersTotal()>=2)
   {
      for(int i=OrdersTotal()-1; i>=0; i--)
      {
        ...
        {
           BuyTicket = OpenBuyOrder(Symbol(), lots[BuyMarketCount(Symbol(), MagicNumber)], 0, MagicNumber, "Alpha Winner V5.00");
           if(BuyTicket>0)
           {
              bool selected = OrderSelect(BuyTicket, SELECT_BY_TICKET);
              double OpenPrice = OrderOpenPrice();
      ...
      for(int i=OrdersTotal()-1; i>=0; i--)
      {
        ...
        {
           SellTicket = OpenSellOrder(Symbol(), lots[SellMarketCount(Symbol(), MagicNumber)], 0, MagicNumber, "Alpha Winner V5.00");
           if(SellTicket>0)
           { 
             bool selected = OrderSelect(SellTicket, SELECT_BY_TICKET);
             double OpenPrice = OrderOpenPrice();
